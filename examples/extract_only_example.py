...
def extract_and_print(pdf_path: str, preprocess: bool = True):
    ...
    successful = sum(1 for k, v in extracted_data.items() if isinstance(k, int) and v is not None)
        ...
        if isinstance(page_num, int):  # Skip error summary
            ...
            if invoice_data:
                ...
                if invoice_data.items:
                    ...
                    print(f"  {'No.':<5} {'Item Name':<30} {'Translated':<30} {'Cost':<15}")
                    print(f"  {'-'*5} {'-'*30} {'-'*30} {'-'*15}")
                    for i, item in enumerate(invoice_data.items, 1):
                        print(f"  {i:<5} {item.name[:30]:<30} {item.name_translated[:30]:<30} {item.cost:<15}")
                else:
